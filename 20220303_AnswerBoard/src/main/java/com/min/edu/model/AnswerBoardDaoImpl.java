...
public class AnswerBoardDaoImpl implements IAnswerBoardDao {
	...
	public boolean signUpInsert(UserVo vo) {
		logger.info(this.getClass().getSigners()+"signUpInsert 회원가입"+vo);
		SqlSession session = manager.openSession(true);
		int cnt = session.insert(NS+"signUpInsert", vo);
		return cnt>0?true:false;
	}
	
	@Override
	public List<UserVo> userInfoAll() {
		logger.info(this.getClass().getSigners()+"userInfoAll 유저 정보 모두");
		SqlSession session = manager.openSession();
		return session.selectList(NS+"userInfoAll");
	}
	
	@Override
	public UserVo loginSelect(UserVo vo) {
		logger.info(this.getClass().getSigners()+"loginSelect 로그인"+vo);
		SqlSession session = manager.openSession();
		return session.selectOne(NS+"loginSelect", vo);
	}
