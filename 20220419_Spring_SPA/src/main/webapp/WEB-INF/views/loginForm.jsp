...
<script type="text/javascript">
	function loginCheck(){
		...
		if(id.value=="" || id.value.trim()==""){
			id.value="";
			id.focus();
			swal("로그인", "ID를 입력해 주세요");
		}else if(pw.value=="" || pw.value.trim()==""){
			pw.value="";
			pw.focus();
			swal("로그인", "PW를 입력해 주세요");
		}else{
			$.ajax({
// 				url:"./loginCheckText.do", //반환되는 값을 Text로 처리
				...
				success:function(msg){
					console.log(msg, typeof msg); //형태확인 꼭 해야함
					if(msg.isc=="성공"){
						console.log(msg.isc, typeof msg.isc);  //성공이라는 글자
						frm.submit();
					}else{
...
<body>
	<fieldset>
		...
		<div id="container">
		<div id="title">SpringSingle Page</div>
			<form method="post">
					<div class="inputWord">아 이 디</div>	
					<input type="text" name="id"  id="inputId" placeholder="아이디 입력" value="GD011"><br>
