...
public class ListBean {
	...
	private String listRowForm(BoardVo boardVo) {
	    ...
	    sb.append("      <a data-toggle='collapse' data-parent='#accordion' href='#collapse"+boardVo.getSeq()+"'>"+replyImage(boardVo.getDepth())+""+boardVo.getTitle()+"</a>  " );
	    sb.append("    </div>                                                                                          " );
	    sb.append("  </td>                                                                                             " );
...
//	    	sb.append("           <input type='button' id='modifyBtn' class='btn btn-primary' value='수정' data-toggle='modal' data-target='modify' onclick=\"modify(\'"+boardVo.getSeq()+"\')\">" );
	    }
	    // 삭제 : 글쓴이거나 관리자일 경우 삭제
	    if(mem.getAuth().equals("A")||mem.getId().equals(boardVo.getId())) {
	    	sb.append("            <input type='button' class='btn btn-primary' value='삭제' onclick=\"del(\'"+boardVo.getSeq()+"\')\">" );
	    }
