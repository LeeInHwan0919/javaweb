...
public class ListBean {
	...
	private String listRowForm(BoardVo boardVo) {
		...
		int index = 1;
			...
			sb.append(" <td>"+(index++)+"</td>" ); //index++의 단점 : 계속 초기화된다
			sb.append(" <td> " );
			...
			sb.append(" </tr>" );
			sb.append(" <tr>" );
			sb.append(" 	<td colspan='"+n+"' style='padding-top: 0px; padding-bottom: 0px;'>" );
			sb.append(" 		<div id='collapse"+boardVo.getSeq()+"' class='panel-collapse collapse'>" );
			sb.append(" 	        <div class='panel-body form-group'>" );
			...
			if(mem.getId().equals(boardVo.getId())) {
				sb.append(" 	       				<input type='button' class='btn btn-primary' value='글 수정' onclick=\"modify(\'"+boardVo.getSeq()+"\')\">" );
			}
			//삭제 내가 쓴글 관리자일때
			if(mem.getId().equals(boardVo.getId()) || mem.getAuth().equals("A")) {
				sb.append(" 	       				<input type='button' class='btn btn-primary' value='글 삭제' onclick=\"delete(\'"+boardVo.getSeq()+"\')\">" );
			}
			//답글 나머지 User일때
			if(mem.getAuth().equals("U")) {
				sb.append(" 	       				<input type='button' class='btn btn-primary' value='글 답글' onclick=\"reply(\'"+boardVo.getSeq()+"\')\">" );
			}
			sb.append(" 	       	</div>" );
			sb.append(" 	       </div>" );
			sb.append(" 	    </div>" );
