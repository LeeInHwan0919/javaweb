...
SELECT MAX(SUBSTR(ID,5))
	FROM SPRINGMEMBER s;


SELECT SUBSTR(ID,4) FROM SPRINGMEMBER s ;
...
-- 사용자의 페이징 처리   
 ...
 UPDATE SPRINGMEMBER 
   ...
   SELECT ID, PW, AUTH, DELFLAG, REGDATE,ROWNUM
FROM(
SELECT ROW_NUMBER() OVER(ORDER BY ID DESC)RM, ID, PW, AUTH, DELFLAG, REGDATE,ROWNUM
   FROM SPRINGMEMBER
      )
   WHERE RM BETWEEN 1 AND 5;
     
  SELECT ID, PW, AUTH, DELFLAG, REGDATE
FROM GD.SPRINGMEMBER;

  
SELECT COUNT(*) FROM SPRINGMEMBER s ;
   

INSERT INTO SPRINGMEMBER (ID,PW,AUTH,DELFLAG,REGDATE) 
	VALUES ('GD00'|| SELECT MAX(TO_NUMBER(SUBSTR(ID,5,3)))+1
					FROM SPRINGMEMBER s ),'1234','U','N',SYSDATE);

SELECT MAX(TO_NUMBER(SUBSTR(ID,5,3)))+1
	FROM SPRINGMEMBER s;
