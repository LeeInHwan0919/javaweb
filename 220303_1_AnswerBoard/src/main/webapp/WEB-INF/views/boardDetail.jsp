...
<body>
	<div id="container">
		<table class="table table-condensed">
			...
			<tr>
				<th class="info">내용2</th><td><textarea rows="5" cols="20" disabled><%=dto.getContent() %></textarea></td>
			</tr>
			<tr>
		...
		<form class="btn-group btn-group-justified">
            <%if(loginInfo != null && loginInfo.getAuth().equalsIgnoreCase("A")){%>
                 <div class="btn-group">
                	<button type="button" class="btn btn-danger" onclick="del(<%=dto.getSeq()%>)">삭제</button>
                 </div>
            <%}%>
            <%if(loginInfo !=null && dto.getId().equalsIgnoreCase(loginInfo.getId())){%>
                 <div class="btn-group">
                    <button type="button" class="btn btn-info" onclick="modify(<%=dto.getSeq()%>)">수정</button>
                  </div>
            <%}%>
                  <div class="btn-group"> 
