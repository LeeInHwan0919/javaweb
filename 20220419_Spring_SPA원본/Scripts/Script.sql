...
INSERT INTO SPRINGBOARD  (SEQ, ID, TITLE, 
            CONTENT, REFER, STEP,
            DEPTH,READCOUNT,DELFLAG , REGDATE)W
...
INSERT INTO SPRINGMEMBER (ID, PW, AUTH, DELFLAG, REGDATE)
	VALUES('G00'||SPRINGMEMBER_ID.NEXTVAL, 1234, 'A', 'Y', SYSDATE);
...
-- 회원리스트 페이징 처리 (강사)
	SELECT ID, ID||'님' NAME, AUTH, DELFLAG, REGDATE, ROW_NUMBER() OVER(ORDER BY REGDATE DESC) RN
	FROM SPRINGMEMBER
	ORDER BY REGDATE DESC
